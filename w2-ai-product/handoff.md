...
```
**CRITICAL PAUSE:** In `03_human_eval.py`, after the first cell runs, it will save programmatic ratings to `assignment_01.xlsx`.
- You MUST stop here, open `assignment_01.xlsx` in Excel/Sheets, and **manually score 10-15 products** for the 5 criteria (`good`, `ok`, or `bad`).
...
- You can compare your MLflow traces via: `mlflow ui --backend-store-uri sqlite:///experiments.db`
...
```
- In Task 5, verify the 5 "Sanity Check" outputs. If the Judge is behaving weirdly, edit the `prompts/judge_all.txt` file until it behaves correctly. Then run the full 51 products.
...
```bash
marimo export html notebooks/01_rubric.py -o task_01.html
marimo export html notebooks/02_generate.py -o task_02.html
marimo export html notebooks/03_human_eval.py -o task_03.html
marimo export html notebooks/04_improve.py -o task_04.html
marimo export html notebooks/05_judge.py -o task_05.html
marimo export html notebooks/06_analysis.py -o task_06.html
```
Submit the HTML files (or just submit the `.py` marimo notebooks directly, depending on the grader's exact preference) alongside the final `assignment_01.xlsx` file.
